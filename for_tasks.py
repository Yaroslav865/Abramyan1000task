...
def for13(N):
    ...
    for i in range(N):
        s += sign * (1.0 + 0.1 * (i + 1))
        sign = -sign                    # чередование знака без if
...
def for22(X, N):
    ...
    for i in range(N + 1):
        s += term
        term *= X / (i + 1)             # X^(i+1) / (i+1)!
    print(f"Значение = {f(s, 6)}   (exp(X) = {f(exp(X), 6)})")


def for23(X, N):
    s = 0.0
    fact = 1.0                          # (2k+1)! при k = 0
    xpow = X                            # X^(2k+1) при k = 0
    sign = 1.0
    for k in range(N + 1):
        s += sign * xpow / fact
        fact *= (2 * k + 2) * (2 * k + 3)
